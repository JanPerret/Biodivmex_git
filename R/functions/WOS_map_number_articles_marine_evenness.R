...
WOS_map_number_articles_marine_evenness <- function(locality_table, subtitle_text, taxa_name) {
  ...
  map <- ggplot(NULL) + 
    ...
    geom_point(data = circle_centroid_joined, aes(x = x, y = y, fill = simpson), shape = 21, size = 70, stroke = 1, col = c("green2", "royalblue1")) + # circles with the filling for the 2 big regions
