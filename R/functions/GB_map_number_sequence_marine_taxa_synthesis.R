...
GB_map_number_sequence_marine_taxa_synthesis <- function(locality_table, subtitle_text, taxa_name, color_scale_mid) {
  ...
  scale_mid <- color_scale_mid/max(locality_table$n_seq_percent, na.rm = TRUE)
  ...
  map <- ggplot(NULL) + 
    ...
    geom_sf(data = med_EEZ_joined, aes(fill = n_seq_percent)) + # EEZ layer with the colour filling
