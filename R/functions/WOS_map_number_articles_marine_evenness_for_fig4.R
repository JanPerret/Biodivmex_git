...
WOS_map_number_articles_marine_evenness_for_fig4 <- function(locality_table, subtitle_text, taxa_name) {
  ...
  fill_scale_limit <- c(min(med_marine_region_joined$simpson, na.rm = TRUE), max(med_marine_region_joined$simpson, na.rm = TRUE))
  ...
  map <- ggplot(NULL) + 
    ...
    geom_sf(data = med_marine_region_joined_under_layer, aes(fill = simpson)) +  # layer with NW med and Tunisian Plateau to avoid overlap
    geom_sf(data = xy.sf.hatch, aes(color = simpson), size = 1.8) + # layer with the 3 small regions that overlap other regions
    geom_sf(data = med_marine_region_joined, aes(fill = simpson)) +  # layer with the other marine regions
    ...
    geom_point(data = circle_centroid_joined, aes(x = x, y = y, fill = simpson), shape = 21, size = 30, stroke = 1, col = c("green2", "royalblue1")) + # circles with the filling for the 2 big regions
    scale_fill_gradientn(name = "Simpson evenness index", colours = palette_evenness, na.value = "white", limits = fill_scale_limit) +
    scale_color_gradientn(name = "Simpson evenness index", colours = palette_evenness, na.value = "white", limits = fill_scale_limit) + # added for the polygons with hatched filling
